...
Measuring the deployed site turned up three things larger than anything the CSS
work had touched. `scripts/bundle-attrib.py` was written for the first of them
and is worth reaching for whenever a bundle looks bigger than it should: it
walks a source map's mappings, charges every span of generated bytes to the file
it came from, and groups the result by npm package.

**jspdf and html2canvas were 68% of main.js.** `PdfDownloadButton` downloads the
PDF from GitHub Releases; those two libraries exist only in
`generatePDFLocally()`, the fallback for when that fetch fails, and the PDF it
makes is a rasterised image whose links do not work. Every visitor was
downloading 68% of the bundle for a path almost none of them reach. A dynamic
`import()` inside that function moves both into chunks fetched on demand:
main.js went from 233 kB to 80 kB gzipped, and the page from 371 kB to 217 kB
over the wire. Verified that the chunks are absent on load and appear only after
clicking download with the Releases API blocked, and that the fallback still
produces a PDF.

**Production builds shipped source maps.** 3.4 MB of a 4.7 MB deploy, with
main.js.map alone at 3.15 MB, publicly readable and pushed into gh-pages history
on every deploy. `.env.production` now sets `GENERATE_SOURCEMAP=false`. Note that
bundle-attrib.py needs the maps, so build with them on before running it.

**The Content Security Policy was written for resources that no longer exist.**
`git log -L` on the line attributes it to 4bc3bcd, "Fix Instagram in-app browser
compatibility issues", where `default-src ... https:` was there to permit the
cdnjs stylesheet and Google Fonts. Both have since been self-hosted, so the
policy now names this origin, with `connect-src` for the Releases lookup and
`'unsafe-inline'` confined to `style-src`, which React's inline style attributes
require. Checked for violations on desktop, under an Instagram in-app user agent
at iPhone dimensions, and through the jsPDF fallback: none in any of the three.
The case that cannot be reproduced headlessly is a real in-app browser injecting
its own scripts, so that still wants a look on a phone.

**Hydration fails and the prerender is wasted on real visitors.** React throws
18 errors on load and client-renders the whole root. `scripts/prerender.js`
captures `document.documentElement.outerHTML`, by which point the browser has
rewritten every inline style: `#2185d0` becomes `rgb(33, 133, 208)`,
`transition: all 0.2s ease` becomes `transition: 0.2s`, a trailing `;` appears.
React compares against its own serialization and bails, and there are 265 inline
style attributes in the prerendered root. Confirmed by re-hydrating the shipped
markup against the development React build, which names the button's `style`
prop. This is not new: the gh-pages commit before today carries the same 265
normalized attributes, so react-snap had the identical flaw. The final DOM is
correct, so the cost is wasted work rather than a wrong render. Fixing it means
prerendering with `renderToString` instead of capturing browser DOM; capturing
the DOM cannot produce markup React accepts while the app uses inline styles.
